...
#include "common.h"
extern bool isSendRecv;
void SendRecvGetCollByteCount(size_t *sendcount, size_t *recvcount, size_t *paramcount,
...
testResult_t SendRecvInitData(struct threadArgs *args, ncclDataType_t type, ncclRedOp_t op,
                              int root, int rep, int in_place)
{
    ...
    for (int i = 0; i < args->nGpus; i++) {
        ...
        TESTCHECK(
            InitData(args->expected[i], recvcount, peer * recvcount, type, ncclSum, rep, 1, 0));
...
{
    double baseBw = (double)(count * typesize) / bwUnit / sec;
...
testResult_t SendRecvRunColl(void *sendbuff, void *recvbuff, size_t count, ncclDataType_t type,
                             ncclRedOp_t op, int root, ncclComm_t comm, mcStream_t stream, struct extendCollArg* extendArgs=nullptr)
{
    ...
    if (enable_extend_api) {
        NCCLCHECK(mcclSendExt(sendbuff, count, type, sendPeer, comm, stream));
        NCCLCHECK(mcclRecvExt(recvbuff, count, type, recvPeer, comm, stream));
    } else {
...
testResult_t SendRecvRunTest(struct threadArgs *args, int root, ncclDataType_t type,
...
struct testEngine sendRecvEngine = {SendRecvGetBuffSize, SendRecvRunTest};

//#pragma weak ncclTestEngine=sendRecvEngine
__attribute__((weak)) testEngine ncclTestEngine = sendRecvEngine;
