...
void ReduceScatterGetCollByteCount(size_t *sendcount, size_t *recvcount, size_t *paramcount,
...
testResult_t ReduceScatterInitData(struct threadArgs *args, ncclDataType_t type, ncclRedOp_t op,
                                   int root, int rep, int in_place)
{
    ...
    for (int i = 0; i < args->nGpus; i++) {
        ...
        CUDACHECK(cudaMemcpy(args->expected[i], args->recvbuffs[i], args->expectedBytes,
                             cudaMemcpyDefault));
        TESTCHECK(
            InitDataReduce(args->expected[i], recvcount, rank * recvcount, type, op, rep, nranks));
        CUDACHECK(cudaDeviceSynchronize());
    }
    return testSuccess;
}

void ReduceScatterGetBw(size_t count, int typesize, double sec, double *algBw, double *busBw,
                        int nranks)
{
...
testResult_t ReduceScatterRunColl(void *sendbuff, void *recvbuff, size_t count, ncclDataType_t type,
                                  ncclRedOp_t op, int root, ncclComm_t comm, mcStream_t stream, struct extendCollArg* extendArgs=nullptr)
{
    if (enable_extend_api)
        NCCLCHECK(mcclReduceScatterExt(sendbuff, recvbuff, count, type, op, comm, stream));
    else
...
testResult_t ReduceScatterRunTest(struct threadArgs *args, int root, ncclDataType_t type,
...
struct testEngine reduceScatterEngine = {ReduceScatterGetBuffSize, ReduceScatterRunTest};

// #pragma weak ncclTestEngine=reduceScatterEngine
__attribute__((weak)) testEngine ncclTestEngine = reduceScatterEngine;