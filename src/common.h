...
#define CUCHECK(cmd)                                                                               \
    do {                                                                                           \
        ...
        if (err != CUDA_SUCCESS) {                                                                 \
            ...
            printf("%s: Test CU failure %s:%d '%s'\n", hostname, __FILE__, __LINE__, errorStr);    \
            return testCudaError;                                                                  \
        }                                                                                          \
    } while (0)
...
struct testColl {
    ...
    void (*getCollByteCount)(size_t *sendcount, size_t *recvcount, size_t *paramcount,
                             ...
                             int nranks);
    testResult_t (*initData)(struct threadArgs *args, ncclDataType_t type, ncclRedOp_t op, int root,
                             int rep, int in_place);
    void (*getBw)(size_t count, int typesize, double sec, double *algBw, double *busBw, int nranks);
    testResult_t (*runColl)(void *sendbuff, void *recvbuff, size_t count, ncclDataType_t type,
                            ncclRedOp_t op, int root, ncclComm_t comm, mcStream_t stream, struct extendCollArg* extendArgs);
};
...
extern struct testColl alltoAllvTest;
extern struct testColl alltoAlldTest;
...
struct threadArgs {
    ...
    int reportErrors;
    extendCollArg** collArgs;
    struct testColl *collTest;

    char headStr[128];
    char resultInPlace[128];
    char resultOutPlace[128];
};

struct extendCollArg {
    size_t* sendcounts;
    size_t* recvcounts;
...
extern testResult_t TimeTest(struct threadArgs *args, ncclDataType_t type, const char *typeName,
                             ncclRedOp_t op, const char *opName, int root);
extern testResult_t InitDataReduce(void *data, const size_t count, const size_t offset,
                                   ncclDataType_t type, ncclRedOp_t op, const uint64_t seed,
                                   const int nranks);
extern testResult_t InitData(void *data, const size_t count, size_t offset, ncclDataType_t type,
                             ncclRedOp_t op, const uint64_t seed, const int nranks, const int rank);
extern void AllocateBuffs(void **sendbuff, void **recvbuff, void **expected, void **expectedHost,
                          size_t nbytes, int nranks);
extern void setupArgs(size_t size, ncclDataType_t type, struct threadArgs *args);
extern testResult_t BenchTime(struct threadArgs *args, ncclDataType_t type, ncclRedOp_t op,
                              int root, int in_place, int size_id, info_log &persize);
extern double bwUnit;
extern char bwUnitStr[8];

// Provided by each coll
extern void print_line_header(size_t size, size_t count, const char *typeName, const char *opName,
                              int root);
