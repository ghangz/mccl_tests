...
void GatherGetCollByteCount(size_t *sendcount, size_t *recvcount, size_t *paramcount,
...
testResult_t GatherInitData(struct threadArgs *args, ncclDataType_t type, ncclRedOp_t op, int root,
...
static int mcclGatherOpt = -1;
testResult_t GatherRunColl(void *sendbuff, void *recvbuff, size_t count, ncclDataType_t type,
                           ncclRedOp_t op, int root, ncclComm_t comm, mcStream_t stream, struct extendCollArg* extendArgs=nullptr)
{
    ...
    if (mcclGatherOpt == -1) {
        if (getenv("MCCL_GATHER_ENABLE")) {
            mcclGatherOpt = atoi(getenv("MCCL_GATHER_ENABLE"));
        } else {
            mcclGatherOpt = 1;
        }
    }
    if (mcclGatherOpt == 1) {
        NCCLCHECK(mcclGather(sendbuff, recvbuff, count, type, root, comm, stream));
    } else {
...
struct testColl gatherTest = {"Gather", GatherGetCollByteCount, GatherInitData, GatherGetBw,
                              GatherRunColl};
...
testResult_t GatherRunTest(struct threadArgs *args, int root, ncclDataType_t type,
...
struct testEngine gatherEngine = {GatherGetBuffSize, GatherRunTest};

// #pragma weak ncclTestEngine=gatherEngine
__attribute__((weak)) testEngine ncclTestEngine = gatherEngine;