...
void prepareInput1(
    ...
    uint64_t seed, intptr_t elt_ix0, mcStream_t stream
  ) {
...
__global__ void verifyPrepared(
  ...
  if(bad) {
    *bad_elt_n += bad;
    //__atomic_fetch_add(bad_elt_n, bad, __ATOMIC_SEQ_CST);
  }
...
void verifyInline1(
    ...
    unsigned tolerance, int64_t *bad_elt_n, mcStream_t stream, int block_n
  ) {
  #define CASE_OP(op) \
    if(rank_n == 1) \
...
void ncclVerifiableVerify(
    ...
    int64_t *bad_elt_n, mcStream_t stream
  ) {
